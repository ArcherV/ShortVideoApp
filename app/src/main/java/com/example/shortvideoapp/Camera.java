...
public class Camera extends CameraDevice.StateCallback {
    ...
    public Camera(Context context, DepthFrameVisualizer depthFrameVisualizer) {
        ...
        previewReader = ImageReader.newInstance(DepthFrameAvailableListener.WIDTH,
                DepthFrameAvailableListener.HEIGHT, ImageFormat.DEPTH16,1);
        mThreadHandler = new HandlerThread("CAMERA2");
        mThreadHandler.start();
        mHandler = new Handler(mThreadHandler.getLooper());
        previewReader.setOnImageAvailableListener(imageAvailableListener, mHandler);
    }

    // Open the front depth camera and start sending frames
    public void openBackDepthCamera() {
        final String cameraId = getBackDepthCameraID();
        openCamera(cameraId);
    }

    private String getBackDepthCameraID() {
        try {
            for (String camera : cameraManager.getCameraIdList()) {
                ...
                boolean backingFront = chars.get(CameraCharacteristics.LENS_FACING) == CameraMetadata.LENS_FACING_FRONT;
