...
public class DepthFrameAvailableListener implements ImageReader.OnImageAvailableListener {
    ...
    private static float RANGE_MIN = 0f;
    private static float RANGE_MAX = 8191.0f;
...
//        Log.e(TAG, "width" + image.getWidth() + "Height" + image.getHeight());
    ...
    private Bitmap convertToRGBBitmap(int[] mask) {
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                int index = y * WIDTH + x;
                bitmap.setPixel(x, y, Color.argb(255, mask[index], 0,0));
            }
        }
        return bitmap;
    }
